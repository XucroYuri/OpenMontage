---
description: 手绘墨线涂鸦动画：角色自我绘制后行走、跳舞或挥手，也可制作机械装置讲解（矢量 → HyperFrames MP4）。
argument-hint: "[要制作的动画]"
---

读取 `skills/creative/ink-theater.md` 和 `ink-theater/README.md`，再制作下述手绘墨线动画。
...
- 字幕使用 **HTML overlay `<div>`**，因为 HyperFrames 不会把 Web 字体应用到 SVG `<text>`。渲染前运行 `npx hyperframes lint` 和 `snapshot`。
