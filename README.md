...
<p align="center">
  ...
  <a href="docs/README.md">中文文档中心</a>
</p>
...
> 想支持 OpenMontage？欢迎[赞助本项目](https://github.com/sponsors/calesthio)。
...
<td><strong>Bloome</strong> 让多个 AI 智能体（Claude、ChatGPT、DeepSeek 等）在同一对话中协作完成智能体视频流水线。它无需配置、运行于云端、支持网页和移动端，还能把配置好的智能体共享给整个团队。<strong><a href="https://bloome.im/app?ref=calesthio&utm_medium=github&utm_source=calesthio-OpenMontage-ivor-202607">试用 Bloome</a></strong>。</td>
</tr>
<tr>
...
<td><strong>Atlas Cloud</strong> 是一套全模态 AI 推理平台，为开发者提供统一的视频生成、图像生成和 LLM API。您无需分别维护多个提供商集成，只需接入一次，即可使用跨模态的 300 多个精选模型。可查看 Atlas Cloud 新推出的 <a href="https://www.atlascloud.ai/coding-plan">coding plan</a> 优惠，以更实惠的预算使用 API。</td>
</tr>
</table>
...
将您的 AI 编程助手变成一个完整的视频制作工作室。用通俗的语言描述您的需求——您的智能体会自动处理研究、脚本编写、素材生成、剪辑以及最终合成。
...
OpenMontage 将语言拆成三个互不混淆的层级，而不是把所有英文机械替换为中文：

| 层级 | 默认行为 | 示例 |
|------|----------|------|
| **交互语言** | 使用简体中文 | 对话、审批、费用、进度、警告、交付说明 |
| **成片内容语言** | 默认跟随交互语言，也可由需求单独指定 | 脚本、旁白、对白、画面文字、字幕、发布文案 |
| **工具工作语言** | 采用对应模型和提供商效果最好的语言与结构 | 图像/视频生成提示词、搜索词、模型专用参数 |

这意味着中文用户可以全程用中文理解和控制制作，但智能体不会为了表面汉化而缩短高质量提示词、翻译品牌/产品专名、替换模型，或改变流水线阶段、工具选择与质量门槛。Schema 字段、枚举值、流水线 ID、工具名、模型名和原始 JSON 会保持规范英文；制作看板只翻译它们的显示标签，并在原始数据区域明确说明这一点。

如果输入素材本身是英文，系统也不会擅自翻译。只有当需求明确包含翻译、本地化或配音时，才会生成目标语言版本，同时保留源语言与目标语言之间的可审计对应关系。

**重要区别：** OpenMontage 不仅能制作基于生成图像的动画视频，也能在免费/开源工作流中使用**真实动态影像**：智能体会从免版税素材库和开放档案建立语料库，检索真正会动的镜头，将其剪入时间线并渲染成片，而不是只让几张静态图片产生位移。
...
> **“重塑你的宇宙 (Reimagine Your Universe)”** — 一部 50 秒竖屏变形短片，让同一视觉创意穿梭于不同物体、时代、材质和尺度。五个生成式动态场景、克制的 Google Chirp 旁白、Pixabay 配乐和定制 HyperFrames 合成，将彼此独立的片段串成一段有完整创作意图的电影之旅。总成本：**约 4 美元**。

<div align="center">
  <video src="https://github.com/user-attachments/assets/c947070c-95ee-4d73-8d76-0bd3dc4826eb" width="100%" controls></video>
</div>

> **“产品焕发生命 (Products Come to Life)”** — 一部基于已批准主视觉静帧制作的 60 秒产品短片。五件硬表面产品分别拆解出内部工程结构，再重新组装；每张静帧同时固定为首尾帧，让模型在不丢失产品身份的前提下创造运动。图生视频、定制音效、旁白与专属合成共同完成整片。

<div align="center">
  <video src="https://github.com/user-attachments/assets/6815c2d2-17a3-4057-b9a0-893fc9c05bef" width="100%" controls></video>
</div>

> **“与 OpenMontage 一起想象无限可能 (Imagine the Possibilities with OpenMontage)”** — 七个生成世界组成的一部纯音乐展示片。三个图像模型负责广告、时尚和微缩世界美术，四个视频模型通过建筑、材质变形、生命温室和生物邂逅扩展旅程。OpenMontage 负责静帧动画化、动态剪辑、统一配乐，并以 Monty the Clapper 收尾。源素材生成成本：**约 5 美元**。

<div align="center">
  <video src="https://github.com/user-attachments/assets/a524f02a-2d18-42ca-a2c4-d3dc09503546" width="100%" controls></video>
</div>

> **“盐如何塑造历史 (How Salt Made History)”** — 一部 100 秒电影级纪录片，讲述这种矿物如何资助帝国、塑造贸易路线、引发革命，并赋予我们“salary（薪资）”一词。真实世界素材与原创旁白交织，蚀刻风标题、词源揭示、动态地图、历史时间线和收束论点均使用手工定制的动态视觉完成。

<div align="center">
  <video src="https://github.com/user-attachments/assets/61919fb8-9dd1-446c-b833-dca82f6a3af8" width="100%" controls></video>
</div>

> **“一句提示词构建完整 3D 世界 (One Prompt Built This Complete 3D World)”** — 一段连续 60 秒的旅程，穿行于同一个连贯且可编辑的奇幻世界。不同地貌区、有人居住的村庄、水道、遗迹、茂密植被和后段揭晓的主地标由带纹理的 3D 素材组装，再通过电影级灯光、氛围音乐和规划好的镜头路径融为一体。

<div align="center">
  <video src="https://github.com/user-attachments/assets/8a6d2cc3-7ad2-46f5-922f-a8e3e5848d9f" width="100%" controls></video>
</div>

> **“虚空神经接口 (VOID — Neural Interface)”** — 仅使用一个 API 密钥 (OpenAI) 制作的产品广告。包含 4 张 AI 生成的图像 (gpt-image-1)、TTS 旁白、自动获取的免版税音乐、通过 WhisperX 生成的词级字幕以及 Remotion 数据可视化。总成本：**0.69 美元**。无需手工制作素材。
...
## 观看制作过程：Backlot 实时故事板

聊天窗口只能告诉您智能体“说了什么”，而 **Backlot 会展示制作实际正在做什么**。这个本地制作看板会随着流水线运行自动填充：阶段依次点亮，脚本以剧本页面呈现，素材生成时场景卡片持续更新，所有提供商决策和成本都清楚记录在看板上。

制作开始后，智能体会自动打开看板。无需额外配置或手工汇报；它直接读取流水线本来就会写入的项目文件。

<p align="center"><img src="docs/images/backlot/board-live.png" alt="Backlot 实时制作看板：正在生成素材" width="920"></p>

**故事板现在也是正式审批门。** 素材生成会停在逐场景候选缩略图总览：其中展示候选版本、提示词、单项成本和质量评分，让您在渲染锁定素材之前批准画面，而不是等成片完成后才发现问题。

<p align="center"><img src="docs/images/backlot/storyboard.png" alt="Backlot 故事板：含候选素材和渲染结果的胶片视图" width="920"></p>

创意审批门会一直等待您的答复。看板会说明正在等待什么以及原因，您只需在聊天中回复：

<p align="center"><img src="docs/images/backlot/script-gate.png" alt="Backlot 脚本审批门：等待用户批准" width="920"></p>

本机上的所有制作都以实时状态优先，并统一显示在项目库中：

<p align="center"><img src="docs/images/backlot/library.png" alt="Backlot 项目库" width="920"></p>

```bash
python -m backlot open                  # 打开本机所有项目的项目库
python -m backlot open <project-id>     # 打开指定项目的实时制作看板
python scripts/backlot_simulate_run.py  # 还没有项目？先观看一次模拟制作
```

制作完成后，点击 **▶ 回放制作**，即可按时间戳重放整个制作过程，并在全程任意拖动查看。工作原理详见 [`backlot/README.md`](backlot/README.md)。
...
想了解更多？继续使用上面的中文示例，或者运行 `make demo` 立即渲染零密钥的演示视频。完整提示词画廊见 [`PROMPT_GALLERY.md`](PROMPT_GALLERY.md)。
...
> 本页“添加 API 密钥”与后续提供商表格已经覆盖中文用户开始制作所需的配置。完整的中文深度参考见 [`docs/PROVIDERS.md`](docs/PROVIDERS.md)。
...
风格 Playbook 控制排版、调色板、运动风格、音频配置和质量规则。智能体读取 Playbook，并将其统一应用到所有生成素材中。
...
<a href="https://www.star-history.com/?repos=calesthio%2FOpenMontage&type=date&legend=top-left">
  <picture>
    ...
    <img alt="OpenMontage Star 历史图表" src="https://api.star-history.com/image?repos=calesthio/OpenMontage&type=date&legend=top-left" />
  </picture>
</a>
...
如果您愿意进一步支持，欢迎[赞助本项目](https://github.com/sponsors/calesthio)。OpenMontage 主要利用夜晚和周末持续开发，您的支持会让这项工作更加可持续。
